...
class DiscretePendulumEnv(gym.Env):
    ...
    def __init__(self, g=10.0, n_th=40, n_thdot=60, num_u=15):
        ...
        self.width_th = 2.0 * np.pi / self.n_th
        self.width_thdot = 2.0 * self.max_speed / self.n_thdot
        self.num_u = num_u
        self.u_interval = 2.0 * self.max_torque / (self.num_u - 1)
    ...
    def reset(self, deterministic=False):
        if deterministic:
            ...
            discrete_state = self.to_discrete(
                self.np_random.uniform(low=-high, high=high)
            )
        self.x = self.to_continuous(discrete_state)
        self.last_u = None
    ...
    def f(self, x, u):
        ...
        newthdot = (
            thdot
            + (-3.0 * g / (2.0 * L) * np.sin(th + np.pi) + 3.0 / (m * L**2) * u) * dt
        )
    ...
    def bin(self, x: np.ndarray) -> List[float]:
        ...
        n2, x2 = (
            int((thdot + self.max_speed) // self.width_thdot),
            (thdot + self.max_speed) % self.width_thdot,
        )
        ...
        if x1 >= x2:
            ...
            d1, d2, d3 = (
                d,
                (d + self.n_thdot + 1) % self.n_pts,
                (d + 2 + self.n_thdot) % self.n_pts,
            )
            p[d1], p[d2], p[d3] = 1.0 - x1, x1 - x2, x2
        else:
